...
def linear_regression_noreg(X, y):
  ...
  """
  X_t = np.transpose(X)
  X_tX = np.linalg.inv(np.matmul(X_t, X))
  X_tXX_t = np.matmul(X_tX, X_t)
  w = np.matmul(X_tXX_t, y)
  return w
...
def regularized_linear_regression(X, y, lambd):
  ...
  temp = X_tX + np.identity(dim[0]) * lambd

  temp_inv = np.linalg.inv(temp)
  w = np.matmul(np.matmul(temp_inv, X_t), y)
...
def tune_lambda(Xtrain, ytrain, Xval, yval, lambds):
  ...
  for lambd in lambds:
    ...
    for i in range(len(Xval)):
      temp = np.square(np.matmul(Xval[i], w) - yval[i])
      RSS += temp
    if flag:
      RSS_temp = RSS
      bestlambda = lambd
...
def test_error(w, X, y):
  ...
  for i in range(len(X)):
      temp = np.square(np.matmul(X[i], w) - y[i])
      RSS += temp
  err = np.divide(RSS, len(X))
  return err
